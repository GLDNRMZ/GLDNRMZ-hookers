...
-- NUI EVENTS
RegisterNetEvent("lb-hookers:OpenPimpMenu")
AddEventHandler("lb-hookers:OpenPimpMenu", function ()
...
RegisterNetEvent("lb-hookers:OpenHookerMenu")
AddEventHandler("lb-hookers:OpenHookerMenu", function ()
    SetNuiFocus(true, true)
    Citizen.Wait(100)
    SendNUIMessage({
        type    = "openHookerMenu",
...
RegisterNUICallback("ChooseCathrine", function (data, callback)
    SetNuiFocus(false, false)
    callback("ok")
    TriggerEvent("lb-hookers:ChosenHooker", "s_f_y_hooker_01") -- Cathrine
...
RegisterNUICallback("ChooseTatiana", function (data, callback)
    SetNuiFocus(false, false)
    callback("ok")
    TriggerEvent("lb-hookers:ChosenHooker", "s_f_y_hooker_02") -- Tatiana
...
RegisterNUICallback("ChooseBootylicious", function (data, callback)
    SetNuiFocus(false, false)
    callback("ok")
    TriggerEvent("lb-hookers:ChosenHooker", "s_f_y_hooker_03") -- Bootylicious
...
RegisterNUICallback("ChooseVennesa", function (data, callback)
    SetNuiFocus(false, false)
    callback("ok")
    TriggerEvent("lb-hookers:ChosenHooker", "s_f_y_hooker_02") -- Vennesa
...
RegisterNUICallback("ChooseBlowjob", function (data, callback)
    ...
    callback("ok")
    HookerInCar = false
    TriggerServerEvent("lb-hookers:pay", true)
...
RegisterNUICallback("ChooseSex", function (data, callback)
    ...
    callback("ok")
    HookerInCar = false
    TriggerServerEvent("lb-hookers:pay", false)
    if math.random(1, 100) >= 10 then
        exports['ps-dispatch']:SuspiciousActivity2()
    end
end)
...
-- No Money 
RegisterNetEvent("lb-hookers:noMoney")
AddEventHandler("lb-hookers:noMoney", function()
    HookerInCar = true
end)
...
RegisterNetEvent("lb-hookers:ChosenHooker")
AddEventHandler("lb-hookers:ChosenHooker", function(model)
                    ...
                    if GetPedInVehicleSeat(vehicle, -1) and IsPedInVehicle(ped, vehicle, true) and not IsVehicleSeatFree(vehicle, 0) and not IsVehicleSeatFree(vehicle, -1) then
                        ...
                        if IsVehicleStopped(vehicle) then
                            ...
                            if IsControlJustPressed(0, Keys["E"]) then
                                ...
                                TriggerEvent("lb-hookers:OpenHookerMenu")
                            end
...
RegisterNetEvent("lb-hookers:startSex")
AddEventHandler("lb-hookers:startSex", function()
    local ped = PlayerPedId()
